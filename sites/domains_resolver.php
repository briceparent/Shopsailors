...
if( isset( $_SESSION[ 'SH_SITE' ] ) && !SH_IS_DEV ) {
    ...
    define( 'SH_MASTERISUSER', $_SESSION[ 'constants' ][ 'SH_MASTERISUSER' ] );
} else {
    $siteFound = false;
    ...
    if( !$siteFound ) {
        ...
        define( 'SH_MASTERISUSER', false );
        $_SESSION[ 'constants' ][ 'SH_MASTERSERVER' ] = SH_MASTERSERVER;
        $_SESSION[ 'constants' ][ 'SH_MASTERISUSER' ] = SH_MASTERISUSER;
    }
}
...
function setMasterServerConstants() {
    ...
    if( is_array( $masterIsUser ) ) {
        define( 'SH_MASTERISUSER', in_array( SH_SITENAME, $masterIsUser ) );
    }else{
        define( 'SH_MASTERISUSER', false);
    }

    if(SH_MASTERISUSER){
        define( 'SH_MASTERSERVER_DOMAIN', $_SERVER['HTTP_HOST'] );
        define( 'SH_MASTERSERVER_SITE', SH_SITE );
    }else{
        if( !SH_MASTERSERVER ) {
            if( !SH_GLOBAL_DEBUG ) {

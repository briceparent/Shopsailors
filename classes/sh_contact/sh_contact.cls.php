...
class sh_contact extends sh_core {
    ...
    public function show() {
        ...
        if( $formSubmitted === true ) {
            ...
            if( $mailOK || $phoneOK ) {
                ...
                $mailer->em_addSubject(
                    $mailObject, $this->linker->path->getBaseUri() . ' - ' . $this->getI18n( 'mail_subject' )
                );
        ...
        if( $this->getParam( 'showAddress' ) && is_array( $this->getParam( 'address' ) ) ) {
            $open = false;
            foreach( $this->getParam( 'address' ) as $oneLine ) {
                if( !empty( $oneLine ) && !$open ) {
                    $address .= '<div class="oneAddress">';
                    $open = true;
                }
                if( empty( $oneLine ) && $open ) {
                    // Starting a new address
                    $address .= '</div>';
                    $open = false;
                }
                if( !empty( $oneLine ) ) {
                    $address .= '<div>' . $oneLine . '</div>';
                }
            }
            if( $open ) {
                // We should close the last address
                $address .= '</div>';
            }
    ...
    public function saveParams() {
        ...
        if( isset( $_POST[ 'contact' ][ 'activated' ] ) ) {
            ...
            $this->addToSitemap(
                'contact/show/', self::SITEMAP_PRIORITY, self::SITEMAP_FREQUENCY
            );
        } else {
        ...
        $this->setParam( 'showPhone', $showPhone );
        //showAddress

...
class sh_shop extends sh_core {
    ...
    public $callWithoutId = array(
        ...
        'get_cart_plugin_ajax','bill_delete'
    );
    ...
    public function bill_delete(){
        $this->onlyAdmin();
        if($this->formSubmitted( 'delete_bill')){
            if(isset($_POST['delete']) || isset($_POST['delete_x'])){
                ...
                $this->linker->path->redirect(
                    $this->linker->path->getLink(__CLASS__.'/showBill/').'?bill_id='.$_GET['bill_id']
                );
            }
        }
        $this->linker->html->setTitle($this->getI18n('bill_delete_title'));
        $commandsFolder = SH_SITE_FOLDER . __CLASS__ . '/commands/';
        $id = $_GET[ 'bill_id' ];
        include($commandsFolder . 'list.php');
        if( isset( $commandList[ $id ] ) && file_exists( $commandsFolder . $commandList[ $id ] . '.php' ) ) {
            ...
            if($this->isAdmin()){
                $values[ 'bill' ][ 'delete' ] = $this->linker->path->getLink(
                        $this->shortClassName . '/bill_delete/'
                    ) . '?bill_id=' . $id;
            }
        ...
        $this->render('bill_delete', $values);
    }

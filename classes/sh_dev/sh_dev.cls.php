...
class sh_dev extends sh_core {
    ...
    public function master_getMenuContent() {
        ...
        $masterMenu[ 'Section Développeur' ][ ] = array(
            'link' => 'dev/configure/', 'text' => 'Configuration du debug', 'icon' => 'picto_tool.png'
        );
            ...
            if( is_dir( SH_DEVPARAMS_PATH . $projectName ) ) {
                ...
                if( !is_dir( SH_DEV_PATH . $projectName ) ) {
                    ...
                    for( $a = 0; $a <= $revisions; $a++ ) {
                        $elements = array_diff(
                            scandir( SH_DEV_PATH . $projectName . '/' . $a ), array( '.', '..', 'description.php' )
                        );
            ...
            foreach( array_keys( $_POST[ 'files' ] ) as $file ) {
                ...
                if( file_exists( $revisionFile ) ) {
                    ...
                    if( !file_exists( SH_ROOT_FOLDER . $fileName ) || $noMoreCommitHere ) {
                        ...
                        if( $this->isTextFile( $fileName ) ) {
                            ...
                            $this->linker->diff->createPatch(
                                ...
                                SH_ROOT_FOLDER . $fileName, $revisionFolder . '/' . $file . '.diff'
                            );
                        } else {
                            // We copy the binary file
                            copy(
                                SH_ROOT_FOLDER . $fileName, SH_DEV_PATH . $project . '/' . $firstRevision . '/' . $file . '.bin'
                            );
                        }
                    ...
                    if( $this->isTextFile( $fileName ) ) {
                        ...
                        copy(
                            SH_ROOT_FOLDER . $fileName, $revisionFolder . $file . '.php'
                        );
                    } else {
                        // We copy the binary file
                        copy(
                            SH_ROOT_FOLDER . $fileName, $revisionFolder . $file . '.bin'
                        );
                    }
                ...
                // We add a line for this revision
                $this->helper->writeInFile(
                    $revisionFile, $newRevision . ' ' . $filemd5 . ' ' . $_POST[ 'text' ][ $file ] . "\n", true
                );
            }
        }
    }
                ...
                if( is_dir( SH_DEV_PATH . $projectName ) ) {
                    ...
                    foreach( array_keys( $existing ) as $missing ) {
                        if( !file_exists( SH_DEV_PATH . $projectName . '/revisions/' . $missing . '.php' ) ) {
                            $revisions = file(
                                SH_DEV_PATH . $projectName . '/revisions/' . $missing, FILE_IGNORE_NEW_LINES
                            );
    ...
    protected function initializeProject( $projectName ) {
        ...
        $this->helper->deleteDir( SH_DEV_PATH . $projectName . '/revisions/' );
        $this->helper->writeInFile(
            SH_DEV_PATH . $projectName . '/actualRevision.txt', '0'
        );
        mkdir( SH_DEV_PATH . $projectName . '/revisions/' );
        mkdir( SH_DEV_PATH . $projectName . '/0/' );

        $this->helper->writeInFile(
            SH_DEV_PATH . $projectName . '/0/description.php', $_POST[ 'description' ]
        );
    ...
    public function configure() {
        ...
        if( $this->formSubmitted( 'dev_configurer' ) ) {
            ...
            if( isset( $_POST[ 'codeCoverage_activated' ] ) ) {
                ...
                $constants .= 'define("SH_DEBUG_COVERAGE_CLASS","' . $_POST[ 'code_coverage_class' ] . '");' . "\n";
            } else {
            ...
            if( isset( $_POST[ 'E_DEPRECATED_activated' ] ) ) {
                $this->setParam( 'errors>e_deprecated', 'checked' );
                if( $operator == 'remove' ) {
                    $errors = $errors ^ E_DEPRECATED;
                } else {
                    $errors = $errors & E_DEPRECATED;
                }
            } else {
                $this->setParam( 'errors>e_deprecated', '' );
            }
...
'
        ...
        $values[ 'default' ][ 'E_DEPRECATED_activated' ] = $this->getParam( 'errors>e_deprecated', '' );
        $values[ 'default' ][ 'E_NOTICE_activated' ] = $this->getParam( 'errors>e_notice', '' );

        foreach( $this->helper->getFolderContents( SH_CLASS_FOLDER, sh_helper::FOLDER_CONTENTS_FOLDERS ) as $folder ) {
            if( in_array( substr( $folder, 0, 3 ), array( 'sh_', 'cm_' ) ) ) {
                $values[ 'code_coverage' ][ ][ 'class' ] = $folder;
            }
        }

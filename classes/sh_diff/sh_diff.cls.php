...
class sh_diff extends sh_core {
    ...
    public function showChanges( $project = null, $file = null, $from = null, $to = null ) {
        ...
        if( file_exists( SH_DEV_PATH . $project . '/revisions/' . $file . '.php' ) ) {
            $this->onlyDiff = false;
            $lines = file(
                SH_DEV_PATH . $project . '/revisions/' . $file . '.php', FILE_IGNORE_NEW_LINES
            );
                ...
                if( count( $lines ) == 1 ) {
                    ...
                    if( $textFile ) {
                        ...
                        $old = $this->applyPatch(
                            ...
                            SH_DEV_PATH . $project . '/' . $lastRevision . '/' . $file . '.diff', true
                        );
                    } else {
            ...
            echo $this->renderDiff(
                $old, SH_ROOT_FOLDER . $filename, false, 4, $values
            );
        } else {
    ...
    protected function makeDiff( $old, $new ) {
        ...
        return array_merge(
                $this->makeDiff(
                    array_slice( $old, 0, $oldMax ), array_slice( $new, 0, $newMax )
                ), array_slice( $new, $newMax, $maxlen ),
                                $this->makeDiff(
                    array_slice( $old, $oldMax + $maxlen ), array_slice( $new, $newMax + $maxlen )
                )
        );
    }
